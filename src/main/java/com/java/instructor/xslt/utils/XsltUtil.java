...
public final class XsltUtil {
	...
	// input XML string and input XSLT string with parameter map added for transforming the content 
	public static String transformXMLString1(String inputXmlString, String inputXsltString, Map<String, Object> params)  {
		StreamSource xmlSourcce = new StreamSource(new StringReader(inputXmlString));
		StreamSource xsltSourcce = new StreamSource(new StringReader(inputXsltString));
		return transformXMLString(xmlSourcce, xsltSourcce, params);

	}

	// input XML Source and input XSLT source with parameter map added for transforming the content 
	public static String transformXMLString(StreamSource xmlSourcce, StreamSource xsltSourcce,
			Map<String, Object> params) {

	System.out.println(">>>>>>>>>>>>>>>>>>>>> XSLT 1.0 transformXMLString(StreamSourcexmlSourcce, StreamSource xsltSourcce, HashMap<String, ?> params") ;
		Reader xsltReader = xsltSourcce.getReader();
		...
		try {
			...
			Transformer transformer = tf.newTransformer(xsltSourcce);

			if (params != null) {
				setXsltParams(params, transformer);
			}

			transformer.setOutputProperty(OutputKeys.INDENT, "yes");
			transformer.transform(xmlSourcce, sr);
	...
	private static void setXsltParams(Map<String, Object> params, Transformer transformer) {
		params.forEach((k, v) -> {
			System.out.println("param Key = " + k + ", Value = " + v);
